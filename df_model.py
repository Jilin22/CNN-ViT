...
# input projection with downsample
class InputProj(nn.Module):
    def __init__(self, in_channel=3, out_channel=32, kernel_size=3, stride=1, act_layer=nn.LeakyReLU):
        super().__init__()
        self.proj = nn.Sequential(
            nn.Conv2d(in_channel, out_channel // 2, 3, stride=stride, padding=kernel_size // 2),
            act_layer(inplace=True),
            nn.Conv2d(out_channel // 2, out_channel // 2, 3, stride=stride, padding=kernel_size // 2),
            act_layer(inplace=True),
        )
        self.down = nn.Sequential(
            nn.Conv2d(out_channel // 2, out_channel, kernel_size=3, stride=stride, padding=1),
            nn.AvgPool2d(2, stride=2),
            act_layer(inplace=True),
        )
        self.in_channel = in_channel
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        x_ori_res = self.proj(x)
        x = self.down(x_ori_res).flatten(2).transpose(1, 2).contiguous()  # B H*W C
        return x_ori_res.flatten(2).transpose(1, 2).contiguous(), x


# Output Projection with upsample
class OutputProj(nn.Module):
    def __init__(self, ch_in=64, ch_out=3):
        super().__init__()
        self.up = nn.Sequential(
            nn.Conv2d(ch_in, ch_in // 2, 3, stride=1, padding=1),
            nn.ConvTranspose2d(ch_in // 2, ch_in // 4, 2, stride=2),
            nn.LeakyReLU(inplace=True)
        )
        self.head = nn.Sequential(
            nn.Conv2d(ch_in // 2, ch_in // 2, 3, stride=1, padding=1),
            nn.Conv2d(ch_in // 2, ch_out * 2, 3, stride=1, padding=1),
            nn.Sigmoid(),
        )
        self.ch_in = ch_in
        self.ch_out = ch_out

    def forward(self, x, x_ori_res):
        # do reshape
        B, L, C = x.shape
        H = int(math.sqrt(L))
        W = int(math.sqrt(L))
        ...
        _, L_ori, C_ori = x_ori_res.shape
        H_ori = int(math.sqrt(L_ori))
        W_ori = int(math.sqrt(L_ori))
        x_ori_res = x_ori_res.transpose(1, 2).contiguous().view(B, C_ori, H_ori, W_ori)

        x = self.up(x)
        flare, output = torch.split(self.head(torch.cat([x, x_ori_res], 1)), 3, dim=1)
        return flare, output
...
class CNN_ViT(nn.Module):
    def __init__(self,
        ...
        # Input/Output
        self.input_proj = InputProj(in_chans, embed_dim, 3, 1, act_layer=nn.LeakyReLU)
        self.output_proj = OutputProj(2 * embed_dim, out_chans)

        # Encoder
        self.ori_res = img_size
    ...
    def forward(self, x, mask=None):
        # Input Projection
        y_ori_res, y = self.input_proj(x)  # B H*W C
        y = self.pos_drop(y)
        # Encoder
